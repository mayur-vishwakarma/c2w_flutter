...
import 'package:adv_todo_app_with_java_backend/models/task_model.dart';
...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> getTodos() async {
    Uri url = Uri.parse("http://192.168.31.240:8080/todo");
    http.Response response = await http.get(url);
    List<dynamic> data = jsonDecode(response.body);
    task = List.generate(
      data.length,
      (index) {
        return Task.fromMap(data[index]);
      },
    );
    setState(() {});
    log(data.toString());
  ...
  Future<Task?> getTodo(int id) async {
    Uri url = Uri.parse("http://192.168.31.240:8080/todo/$id");
    http.Response response = await http.get(url);
    List<dynamic> data = jsonDecode(response.body);
  ...
  void submitForm(bool isEdit, [Task? updateTask]) async {
    if (_formKey.currentState!.validate()) {
      if (isEdit && updateTask != null) {
        ...
        updateTask.date = dateController.text.trim().toString();
        updateTask.ischecked = updateTask.ischecked;
        await updateTodo(updateTask);
